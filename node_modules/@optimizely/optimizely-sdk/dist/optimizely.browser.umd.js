    ...
    ***************************************************************************** */
    ...
    var OptimizelyUserContext = /** @class */ (function () {
        ...
        OptimizelyUserContext.prototype.setForcedDecision = function (context, decision) {
            ...
            if (!this.optimizely.isValidInstance()) {
                logger$1.error(DECISION_MESSAGES.SDK_NOT_READY);
                return false;
            }
         ...
         */
        OptimizelyUserContext.prototype.findValidatedForcedDecision = function (flagKey, ruleKey) {
            var decideReasons = [];
            var forcedDecision = this.findForcedDecision({ flagKey: flagKey, ruleKey: ruleKey });
            var variation = null;
            var variationKey;
            if (forcedDecision) {
                ...
                if (variation) {
                    if (ruleKey) {
                        logger$1.info(LOG_MESSAGES.USER_HAS_FORCED_DECISION_WITH_RULE_SPECIFIED, variationKey, flagKey, ruleKey, this.userId);
                        decideReasons.push([
                        ...
                        logger$1.info(LOG_MESSAGES.USER_HAS_FORCED_DECISION_WITH_NO_RULE_SPECIFIED, variationKey, flagKey, this.userId);
                        decideReasons.push([
                    ...
                    if (ruleKey) {
                        logger$1.info(LOG_MESSAGES.USER_HAS_FORCED_DECISION_WITH_RULE_SPECIFIED_BUT_INVALID, flagKey, ruleKey, this.userId);
                        decideReasons.push([
                        ...
                        logger$1.info(LOG_MESSAGES.USER_HAS_FORCED_DECISION_WITH_NO_RULE_SPECIFIED_BUT_INVALID, flagKey, this.userId);
                        decideReasons.push([
    ...
    function splitVersion(version) {
        ...
        if (hasWhiteSpaces(version)) {
            logger$3.warn(LOG_MESSAGES.UNKNOWN_MATCH_TYPE, MODULE_NAME$4, version);
            return null;
        }
    ...
    function evaluate$1(condition, userAttributes) {
        ...
        if (typeof conditionMatch !== 'undefined' && MATCH_TYPES.indexOf(conditionMatch) === -1) {
            logger$4.warn(LOG_MESSAGES.UNKNOWN_MATCH_TYPE, MODULE_NAME$5, JSON.stringify(condition));
            return null;
        }
        var attributeKey = condition.name;
        if (!userAttributes.hasOwnProperty(attributeKey) && conditionMatch != EXISTS_MATCH_TYPE) {
            logger$4.debug(LOG_MESSAGES.MISSING_ATTRIBUTE_VALUE, MODULE_NAME$5, JSON.stringify(condition), attributeKey);
            return null;
        }
    ...
    function exactEvaluator(condition, userAttributes) {
        ...
        if (!isValueTypeValidForExactConditions(conditionValue) ||
            ...
            logger$4.warn(LOG_MESSAGES.UNEXPECTED_CONDITION_VALUE, MODULE_NAME$5, JSON.stringify(condition));
            return null;
        }
        if (userValue === null) {
            logger$4.debug(LOG_MESSAGES.UNEXPECTED_TYPE_NULL, MODULE_NAME$5, JSON.stringify(condition), conditionName);
            return null;
        }
        if (!isValueTypeValidForExactConditions(userValue) || conditionValueType !== userValueType) {
            logger$4.warn(LOG_MESSAGES.UNEXPECTED_TYPE, MODULE_NAME$5, JSON.stringify(condition), userValueType, conditionName);
            return null;
        }
        if (fns.isNumber(userValue) && !fns.isSafeInteger(userValue)) {
            logger$4.warn(LOG_MESSAGES.OUT_OF_BOUNDS, MODULE_NAME$5, JSON.stringify(condition), conditionName);
            return null;
        }
    ...
    var AudienceEvaluator = /** @class */ (function () {
        ...
        AudienceEvaluator.prototype.evaluate = function (audienceConditions, audiencesById, userAttributes) {
            ...
            var evaluateAudience = function (audienceId) {
                ...
                if (audience) {
                    logger$5.log(LOG_LEVEL.DEBUG, LOG_MESSAGES.EVALUATING_AUDIENCE, MODULE_NAME$6, audienceId, JSON.stringify(audience.conditions));
                    ...
                    logger$5.log(LOG_LEVEL.DEBUG, LOG_MESSAGES.AUDIENCE_EVALUATION_RESULT, MODULE_NAME$6, audienceId, resultText);
                    return result;
                }
                return null;
            };
        ...
        AudienceEvaluator.prototype.evaluateConditionWithUserAttributes = function (userAttributes, condition) {
            ...
            if (!evaluator) {
                logger$5.log(LOG_LEVEL.WARNING, LOG_MESSAGES.UNKNOWN_CONDITION_TYPE, MODULE_NAME$6, JSON.stringify(condition));
                return null;
            }
            try {
                ...
                logger$5.log(LOG_LEVEL.ERROR, ERROR_MESSAGES.CONDITION_EVALUATOR_ERROR, MODULE_NAME$6, condition.type, err.message);
            }
            return null;
        };
    ...
    var DecisionService = /** @class */ (function () {
        ...
        DecisionService.prototype.getVariationFromExperimentRule = function (configObj, flagKey, rule, user, options) {
            ...
            var forcedDecisionResponse = user.findValidatedForcedDecision(flagKey, rule.key);
     ...
     * limitations under the License.
     */
    var logger$6 = lib_2$1('EVENT_BUILDER');
    /**
    ...
    var buildConversionEvent = function (_a) {
        ...
        var revenue = eventTags ? getRevenueValue(eventTags, logger$6) : null;
        var eventValue = eventTags ? getEventValue(eventTags, logger$6) : null;
        return {
            type: 'conversion',
            timestamp: fns.currentTimestamp(),
            uuid: fns.uuid(),
    ...
    function buildVisitorAttributes(configObj, attributes) {
        ...
        if (attributes) {
            Object.keys(attributes || {}).forEach(function (attributeKey) {
                var attributeValue = attributes[attributeKey];
                if (isAttributeValid(attributeKey, attributeValue)) {
                    var attributeId = getAttributeId(configObj, attributeKey, logger$6);
                    if (attributeId) {
    ...
    var Optimizely = /** @class */ (function () {
        ...
        Optimizely.prototype.decide = function (user, key, options) {
            ...
            var forcedDecisionResponse = user.findValidatedForcedDecision(key);
         ...
         */
        Optimizely.prototype.getFlagVariationByKey = function (flagKey, variationKey) {
            var configObj = this.projectConfigManager.getConfig();
            if (!configObj) {
                return null;
            }
            var variations = configObj.flagVariationsMap[flagKey];
    ...
    var createInstance = function (config) {
        try {
            ...
            if (!eventProcessorConfigValidator.validateEventBatchSize(config.eventBatchSize)) {
                logger$7.warn('Invalid eventBatchSize %s, defaulting to %s', config.eventBatchSize, DEFAULT_EVENT_BATCH_SIZE);
                eventBatchSize = DEFAULT_EVENT_BATCH_SIZE;
            }
            if (!eventProcessorConfigValidator.validateEventFlushInterval(config.eventFlushInterval)) {
                logger$7.warn('Invalid eventFlushInterval %s, defaulting to %s', config.eventFlushInterval, DEFAULT_EVENT_FLUSH_INTERVAL);
                eventFlushInterval = DEFAULT_EVENT_FLUSH_INTERVAL;
            }
            var errorHandler = lib_7$1();
            var notificationCenter = createNotificationCenter({ logger: logger$7, errorHandler: errorHandler });
            ...
            var optimizelyOptions = __assign(__assign({ clientEngine: JAVASCRIPT_CLIENT_ENGINE }, config), { eventProcessor: eventProcessor$1.createEventProcessor(eventProcessorConfig), logger: logger$7,
                errorHandler: errorHandler, datafileManager: config.sdkKey ? createHttpPollingDatafileManager(config.sdkKey, logger$7, config.datafile, config.datafileOptions) : undefined, notificationCenter: notificationCenter });
            ...
            try {
                ...
                logger$7.error(LOG_MESSAGES.UNABLE_TO_ATTACH_UNLOAD, MODULE_NAME$e, e.message);
            }
            return optimizely_1;
        }
        catch (e) {
